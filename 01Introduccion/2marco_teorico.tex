...
Para el desarrollo de este trabajo se abordan temas como la obtención de la informacion en una ecena (visión por computadora), clasificación (RANSAC) y la implementación usando AGC.  Por lo cual, a continuación, se describen los conceptos necesarios para su desarrollo.

\subsection{Representación Digital}

Una imagen digital es guardada en una computadora usando formatos de imagen (TIFF, BMP, JPEG, etc.), pero todas las imágenes luego ser interpretadas son representadas en una pantalla por un arreglo bidimensional de \glspl{pixel}.\\

El arreglo de píxeles se puede ver como un conjunto de matrices, dependiendo del modelo del color usado en la representación del píxel (RGB, CMY, RGBA, CMYA, etc.) son los canales de colores o umero de bits necesarios para representarlo por completo. Así una imagen en RGB (rojo, verde, azul por sus siglas en inglés) cuenta con tres canales de color, cada canal define la intensidad de cada color que al mesclarse crean el color deseado, así para mostrar una imagen $MxN$ en RGB es necesario tres matrices de $M$ filas y $N$ columnas, una que define la intencidad del color rojo, otra para el verde y la ultima para el azul.\\

Usando el concepto de que una imagen se representa como una matriz, es posible generar una imagen nueva a partir de otras realizando operaciones como la suma, resta y multiplicación, aplicando una transformación como se muestra en la ecuación \eqref[ec.]{eq:transfimg}.\\
...
Si una imagen es una matriz bidimencional, entonces una matriz tridimencional genera una \gls{nube de puntos}, esta se describe como el conjunto de puntos en un espacio dado o una estructura de datos usada para representar una colección de puntos \cite{weinmann2016reconstruction}. Al trabajar con nubes de puntos se pueden pensar en ellas como una imagen de tres dimensiones y en lugar de un píxel la unidad se le denomina \gls{punto}. Asi como el uso de transformaciones al igual que las imágenes.\\
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.4\textwidth]{01Introduccion/imagenes/wiiMoe.JPG}
	\caption{Ejemplo de una nube de puntos simple.} 
	\label{fig:nuveDePuntos}
\end{figure}

%agregar imagenes de las diferentes nubes de puntos
...
Desafortunadamente este proceso implica una complejidad muy alta al momento de intentar trabajar con tres dimenciones, e implica un poder de computo muy alto para realizarlo por lo que se descarto el uso de este en el proyecto.
