...
Donde $p_1$ es un píxel de la imagen 1 y $p_2$ un píxel de la imagen 2, $f$ es una función de transformación, y $q(x,y)$ el píxel resultante de la nueva imagen \cite{martinsanz2007ejercicios}.\\

Si una imagen es una matriz bidimencional, entonces una matriz tridimencional genera una \gls{nube de puntos}, esta se describe como el conjunto de puntos en un espacio dado o una estructura de datos usada para representar una colección de puntos \cite{weinmann2016reconstruction}. Al trabajar con nubes de puntos se pueden pensar en ellas como una imagen de tres dimensiones y en lugar de un píxel la unidad se le denomina \gls{punto}. Así es posible el uso de transformaciones al igual que con las imágenes.\\
...
Un punto esta definido por su posición en el espacio, aunque también puede contener información de color y brillo similar a un pixel.
%agregar imagenes de las diferentes nubes de puntos

\subsection{Sensor Kinect}

En el desarrollo de vídeo juegos, la competencia entre grandes empresas ha dado como resultado en una variedad de dispositivos que le permiten al usuario una interacción natural con los entornos virtuales de los vídeo juegos. Estos dispositivos son variados tanto en sus componentes físicos como en la forma de interacción. unos de los dispositivos más conocidos son el Wii Mote, PS/Move y el Kinect.\\

Para la consola Wii de Nintendo se usa un Wii Mote, mostrado en la figura \ref{fig:WiiMote}, que, con el uso de una barra de leds infrarrojos y sensores de posición y aceleración, es capaz de obtener su ubicación y orientación respecto la barra de leds colocada sobre la televisión \cite{UsodelKi56:online}.\\
\begin{figure}[!htb]
	\centering
...
Para que la computadora sea capaz de reconocer su entorno existen diferentes técnicas como: la selección de puntos característicos o marcadores en un objeto, %visto por dos o más cámaras y el cálculo de su posición usando las imágenes obtenidas;
...
El algoritmo usado para la transformada de Hough (TH) fue patentado en 1962 por P. Hough como un método para reconocer líneas rectas y más tarde se extendió para detectar figuras complejas, como círculos, polígonos o elipses, y se le llamó la transformada de Hough generalizada, esta técnica permite la detección de los parámetros de un modelo usando un sistema de votación en el cual cada dato favorece al conjunto de parámetros que satisfacen al modelo para dicho dato,  como desventaja la TH se ha visto que tiende a incrementar la complejidad en modelos con gran cantidad de incógnitas\cite{CompVisionKinecthough}. \\
